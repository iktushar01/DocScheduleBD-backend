...
import { deleteFileFromCloudinary, uploadFileToCloudinary } from "../../config/cloudinaryConfig";
...
const givePrescription = async (user : IRequestUser, payload : ICreatePrescriptionPayload) => {
    ...
    if(appointmentData.doctorId !== doctorData.id){
        throw new AppError(StatusCodes.BAD_REQUEST, "You can only give prescription for your own appointments");
    }
    ...
    if (isAlreadyPrescribed) {
        throw new AppError(StatusCodes.BAD_REQUEST, "You have already given prescription for this appointment. You can update the prescription instead.");
    }
...
const updatePrescription = async (user: IRequestUser, prescriptionId: string, payload: any) => {
    ...
    if (!(user?.email === prescriptionData.doctor.email)) {
        throw new AppError(StatusCodes.BAD_REQUEST, "This is not your prescription!")
    }
