...
const giveReview = async (user : IRequestUser, payload : ICreateReviewPayload) => {
   ...
   const appointmentData = await prisma.appointment.findUniqueOrThrow({
    ...
    if(appointmentData.paymentStatus !== PaymentStatus.PAID){
        throw new AppError(StatusCodes.BAD_REQUEST, "You can only review after payment is done");
    };

    if(appointmentData.patientId !== patientData.id){
        throw new AppError(StatusCodes.BAD_REQUEST, "You can only review for your own appointments");
    };
    ...
    if (isReviewed) {
        throw new AppError(StatusCodes.BAD_REQUEST, "You have already reviewed for this appointment. You can update your review instead.");
    };   
...
const myReviews = async (user: IRequestUser) => {
    ...
    if (!isUserExist) {
        throw new AppError(StatusCodes.BAD_REQUEST, "Only patients can view their reviews");
    }
...
const updateReview = async (user: IRequestUser, reviewId: string, payload: IUpdateReviewPayload) => {
    ...
    if (!(patientData.id === reviewData.patientId)) {
        throw new AppError(StatusCodes.BAD_REQUEST, "This is not your review!")
    }
    const result = await prisma.$transaction(async (tx) => {
