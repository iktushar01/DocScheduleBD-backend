...
const initiatePayment = async (appointmentId: string, user : IRequestUser) => {
    ...
    if(!appointmentData.payment){
        throw new AppError(StatusCodes.NOT_FOUND, "Payment data not found for this appointment");
    }

    if(appointmentData.payment?.status === PaymentStatus.PAID){
        throw new AppError(StatusCodes.BAD_REQUEST, "Payment already completed for this appointment");
    };

    if(appointmentData.status === AppointmentStatus.CANCELED){
        throw new AppError(StatusCodes.BAD_REQUEST, "Appointment is canceled");
    }
